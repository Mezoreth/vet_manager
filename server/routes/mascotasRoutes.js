...
import * as MascotaQueries from '../queries/queriesMascota.js'; // Ajusta la ruta según tu estructura
...
router.get('/', async (req, res) => {
    try {
        const mascotas = await MascotaQueries.getMascotas(); // Llama a la función que obtiene los clientes
        res.json(mascotas); // Devuelve los clientes en formato JSON
    } catch (error) {
...
router.delete('/:id_mascota', async (req, res) => {
  const { id_mascota } = req.params;  // Obtener el id_cliente desde los parámetros de la URL
